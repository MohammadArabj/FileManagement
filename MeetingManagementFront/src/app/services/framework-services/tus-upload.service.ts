...
export class TusUploadService {
  ...
  private async batchLoadFileInfo(guids: string[]): Promise<(AttachmentMetaDto | null)[]> {
    return firstValueFrom(
      this.http
        ...
        .pipe(
          map(result => result || []),
          map(items => guids.map(guid => items.find(i => i.guid === guid) || null)),
  ...
  async uploadFile(fileId: string, options: UploadOptions): Promise<string | null> {
    ...
    try {
      ...
      const initResponse = await firstValueFrom(
        this.http.post<ApiResponse<InitiateUploadResult>>(
          ...
          onSuccess: async () => {
            ...
            try {
              const completeResponse = await firstValueFrom(
                this.http.post<ApiResponse<CompleteUploadResult>>(
    ...
    if (file.sessionGuid) {
      try {
        await firstValueFrom(
          this.http.post(
            `${this.uploadApiUrl}/Cancel/${file.sessionGuid}`,
            {},
            this.buildAuthOptions()
          )
        );
  ...
  async deleteFile(fileId: string): Promise<boolean> {
    ...
    try {
      await firstValueFrom(
        this.http.post(
          `${this.attachmentUrl}/Delete/${file.fileGuid}`,
          {},
          this.buildAuthOptions()
        )
      );
  ...
  private buildAuthOptions(extra: Record<string, any> = {}): Record<string, any> {
